...
```{r}
by_date <- group_by(activity_dataset, date)
total_steps_per_day <- summarise(by_date, steps = sum(steps, na.rm = TRUE))
```
...
```{r}
by_interval <- group_by(activity_dataset, interval)
...
xyplot(steps ~ interval | weekday, totals_combined, type = "l", layout = c(1, 2), xlab = "Interval", ylab = "Steps")
